...
export function AdminDealershipsClient({ dealers, oems, totalCount, initialSearch }: Props) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/admin/dealerships?search=${encodeURIComponent(search)}`);
  };

  const handleDeactivate = async (id: string) => {
    if (!confirm('Are you sure you want to deactivate this Dealership?')) return;
      ...
      <div className="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-700">
            A list of all Dealerships participating in the ACS network.
          </p>
        </div>
        <div className="mt-4 sm:mt-0 flex gap-4">
          ...
          <AddEntityButton page="dealerships" oems={oems} />
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-300">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Parent OEM</th>
              ...
              dealers.map((dealer) => (
                <tr key={dealer.id}>
                  ...
                  <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                    {dealer.oem?.name || 'Unknown'}
                  </td>
                  <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                    {dealer.contact_email || 'N/A'}<br />
                  ...
                  <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                    {dealer.status === 'ACTIVE' && (
                      <button 
                        ...
                        disabled={isDeactivating === dealer.id}
                        className="text-red-600 hover:text-red-900 ml-4 disabled:opacity-50"
                      >
                        {isDeactivating === dealer.id ? '...' : 'Deactivate'}
                      </button>
                    )}
                  </td>
                </tr>
