...
export default function AdminInstallationsPage() {
  const { installations, customers, dealers, partners, technicians, verifyInstallation, rejectInstallation, assignPartner } = useData();
  ...
  const getStatusColor = (status: InstallationStatus) => {
    switch (status) {
      ...
      case 'UNDER VERIFICATION': return 'bg-yellow-100 text-yellow-800 border-yellow-300 border-2';
      case 'REVISIT REQUIRED': return 'bg-red-100 text-red-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  };

  const customer = customers.find(c => c.id === selectedInst?.customerId);
  const dealer = dealers.find(d => d.id === selectedInst?.dealerId);
  const partner = partners.find(p => p.id === selectedInst?.partnerId);
  const technician = technicians.find(t => t.id === selectedInst?.technicianId);
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {installations.map((inst) => {
              const c = customers.find(x => x.id === inst.customerId);
              const d = dealers.find(x => x.id === inst.dealerId);
              const p = partners.find(x => x.id === inst.partnerId);
              ...
              <div className="w-screen max-w-2xl">
                <div className="h-full flex flex-col bg-white shadow-xl overflow-y-scroll">
                  ...
                  <div className="relative flex-1 px-4 py-6 sm:px-6 space-y-6">
                    ...
                    {selectedInst.status === 'UNDER VERIFICATION' && (
                      ...
                      <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            ...
                            {partners.map(p => <option key={p.id} value={p.id}>{p.name} ({p.serviceRegions.join(', ')})</option>)}
                          </select>
                          <button 
                            onClick={() => {
                              if (selectedPartnerId) { assignPartner(selectedInst.id, selectedPartnerId); setSelectedInst(null); }
                            }}
                    ...
                    <div className="mt-8 border-t pt-6">
                      ...
                      <div className="relative border-l-2 border-gray-200 ml-3 space-y-8">
                        ...
                        <div className="relative pl-6">
                          ...
                          <p className="text-xs text-gray-500">{new Date(selectedInst.dateCreated).toLocaleDateString()}</p>
                        </div>
                        ...
                        <div className={`relative pl-6 ${selectedInst.status !== 'NEW' ? 'opacity-100' : 'opacity-40'}`}>
                          ...
                          {selectedInst.partnerId && <p className="text-xs text-gray-500">{partner?.name}</p>}
                        </div>

                        {/* 3. Tech Assigned */}
                        <div className={`relative pl-6 ${['TECHNICIAN ASSIGNED', 'IN PROGRESS', 'UNDER VERIFICATION', 'REVISIT REQUIRED', 'VERIFIED', 'COMPLETED'].includes(selectedInst.status) ? 'opacity-100' : 'opacity-40'}`}>
                          <span className={`absolute -left-2.5 top-1 h-5 w-5 rounded-full border-2 border-white ${['TECHNICIAN ASSIGNED', 'IN PROGRESS', 'UNDER VERIFICATION', 'REVISIT REQUIRED', 'VERIFIED', 'COMPLETED'].includes(selectedInst.status) ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                          <h4 className="text-sm font-semibold text-gray-900">Technician Assigned</h4>
                          {selectedInst.technicianId && <p className="text-xs text-gray-500">{technician?.name}</p>}
                        </div>

                        {/* 4. Installation Submitted */}
                        <div className={`relative pl-6 ${['UNDER VERIFICATION', 'REVISIT REQUIRED', 'VERIFIED', 'COMPLETED'].includes(selectedInst.status) ? 'opacity-100' : 'opacity-40'}`}>
                          <span className={`absolute -left-2.5 top-1 h-5 w-5 rounded-full border-2 border-white ${['UNDER VERIFICATION', 'REVISIT REQUIRED', 'VERIFIED', 'COMPLETED'].includes(selectedInst.status) ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                          <h4 className="text-sm font-semibold text-gray-900">Installation Submitted</h4>
                          {selectedInst.completedAt && <p className="text-xs text-gray-500">{new Date(selectedInst.completedAt).toLocaleDateString()}</p>}
                        </div>
                    ...
                    {/* Technician Checklist & Photos */}
                    {selectedInst.checklist && (
                      <div className="mt-8 border-t pt-6">
                        <h3 className="text-lg font-medium text-gray-900 mb-4">Technician Checklist</h3>
                        <ul className="space-y-3">
                          {selectedInst.checklist.map((item, idx) => (
                            <li key={idx} className="flex justify-between text-sm">
                              <span className="text-gray-700">{item.label}</span>
                              <span className={`font-bold ${item.status === 'YES' ? 'text-green-600' : item.status === 'NO' ? 'text-red-600' : 'text-gray-500'}`}>
                                {item.status}
                              </span>
                            </li>
                          ))}
                    ...
                    {selectedInst.photos && (
                      <div className="mt-8 border-t pt-6">
                        <h3 className="text-lg font-medium text-gray-900 mb-4">Installation Evidence</h3>
                        <div className="grid grid-cols-2 gap-4">
                          {selectedInst.photos.map((photo, idx) => (
                            <div key={idx}>
                              <img src={photo.url} alt={photo.category} className="w-full h-32 object-cover rounded-md border" />
                    ...
                    {selectedInst.rejectionReason && (
                       <div className="mt-8 border-t pt-6">
                          ...
                          <p className="text-sm text-gray-900">{selectedInst.rejectionReason}</p>
                       </div>
                    )}
