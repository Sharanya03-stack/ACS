...
import { deactivateTechnician } from './actions';
...
export function PartnerTechniciansClient({ technicians, totalCount, initialSearch }: Props) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/partner/technicians?search=${encodeURIComponent(search)}`);
  };

  const handleDeactivate = async (id: string) => {
    if (!confirm('Are you sure you want to deactivate this Technician?')) return;
    setIsDeactivating(id);
    const res = await deactivateTechnician(id);
      ...
      <div className="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-700">
            A list of all technicians working under your organization.
          </p>
        </div>
        <div className="mt-4 sm:mt-0 flex gap-4">
          ...
          <AddEntityButton page="technicians" />
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-300">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {technicians.length === 0 ? (
              <tr>
                <td colSpan={6} className="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-500 text-center sm:pl-6">
