...
async function run() {
  ...
  // Fetch users to login via JWT impersonation directly (avoids missing passwords)
  ...
  async function getClientForRole(roleName) {
    ...
    const { error } = await c.auth.signInWithPassword({ email: user.email, password: 'password123' });
    if (error) throw new Error(`Failed to login ${roleName}: ${error.message}`);
    return { client: c, prof, user };
  }
  ...
  let otherOemId = otherOems && otherOems.length > 0 ? otherOems[0].id : null;
  if (!otherOemId) {
     const res = await serviceClient.from('organizations').insert({name:'O2', type:'OEM'}).select().single();
     if(res.error) throw res.error;
     otherOemId = res.data.id;
  }
  
  const { data: otherDealers } = await serviceClient.from('organizations').select('id').eq('type', 'DEALER').neq('id', dealerId).limit(1);
  let otherDealerId = otherDealers && otherDealers.length > 0 ? otherDealers[0].id : null;
  if (!otherDealerId) {
     const res = await serviceClient.from('organizations').insert({name:'D2', type:'DEALER', parent_org_id: oemId}).select().single();
     if(res.error) throw res.error;
     otherDealerId = res.data.id;
  }
  ...
  if (!myCust) {
    const { data: nc, error } = await serviceClient.from('customers').insert({ name: 'T1', email: 't1_'+Date.now()+'@e.com', phone: '1'+Date.now(), dealer_id: dealerId, address: 'Test 1' }).select().single();
  ...
  if (!otherCust) {
    const { data: nc, error } = await serviceClient.from('customers').insert({ name: 'T2', email: 't2_'+Date.now()+'@e.com', phone: '2'+Date.now(), dealer_id: otherDealerId, address: 'Test 2' }).select().single();
    if(error) console.log(error);
    otherCust = nc;
  }
  ...
  if (otherVehicle) {
    ...
    report['D.6 OEM cannot update another OEM vehicle'] = res6.error || !res6.data ? 'PASS' : 'FAIL (Allowed!)';
  }
  ...
  let res7 = null;
  if (!res1.error) {
    res7 = await adminClient.from('chargers').insert({
    ...
    report['D.7 ACS_ADMIN can INSERT valid charger'] = 'FAIL (res1 failed)';
  }
  ...
  if (res7 && !res7.error) {
    ...
    report['D.8 ACS_ADMIN can UPDATE valid charger'] = 'FAIL (res7 failed)';
  }
  ...
  const { data: dVeh, error: dVehErr } = await dealerClient.from('vehicles').insert({
    vin: 'D_VIN_'+Date.now(), oem_id: oemId, dealer_id: dealerId, customer_id: myCust.id, model: 'X'
  }).select().single();
  
  if (dVehErr || !dVeh) {
    report['D.10 Dealer can INSERT valid charger'] = `FAIL (failed to insert dealer vehicle: ${dVehErr?.message})`;
  } else {
    ...
    if (!res10.error) {
      ...
      report['D.11 Dealer can UPDATE valid charger'] = 'FAIL (res10 failed)';
    }
    ...
    report['D.14 Unauthorized roles cannot mutate chargers'] = (res14a.error && (res14b.error || !res14b.data)) ? 'PASS' : 'FAIL (Allowed!)';
  }
