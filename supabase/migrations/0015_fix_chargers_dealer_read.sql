...
CREATE POLICY "Chargers Read Access" ON public.chargers FOR SELECT USING (
    deleted_at IS NULL AND (
        ...
        (public.get_auth_role() = 'DEALER' AND customer_id IN (SELECT id FROM public.customers WHERE dealer_id = public.get_auth_org_id())) OR
        id IN (
            SELECT i.charger_id FROM public.installations i
            WHERE 
